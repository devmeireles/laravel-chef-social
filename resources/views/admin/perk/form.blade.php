<x-admin-layout>
    <div class="flex flex-col">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{route('admin.perk.list')}}"
                                class="ml-1 text-sm font-medium text-gray-800 hover:text-blue-600 md:ml-2">Perks</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
        ...
        <div class="w-100 mt-12 block p-6 bg-white border border-gray-200 rounded-lg">
            <form method="post"
                action="{{isset($item) ? route('admin.perk.update', $item->id) : route('admin.perk.store')}}">
                @csrf

                <div class="w-8/12">
                    <div class="mb-6">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Perk Name</label>
                        <input type="text" id="name" name="name"
                            ...
                            placeholder="The perk name" value="{{!empty($item->name) ? $item->name : old('name')}}">
                    ...
                    <div class="mb-6">
                        ...
                        <select id="countries" name="type"
                            ...
                            @foreach($perkOptions as $option)
                            <option value="{{$option->value}}" {{isset($item) && $option->value === $item->type->value ? 'selected' : null}}
                                >{{Str::title($option->name)}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="slug" name="slug"
                            class="bg-stone-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                            placeholder="The perk name" readonly
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="description" rows="4" name="description"
                            ...
                            placeholder="A short description about the perk">{{!empty($item->description) ? $item->description : old('description')}}</textarea>
                ...
                <div class="w-8/12 flex mt-14">
                    ...
                    @if(isset($item) && $item->deleted_at)
                    <a href="{{route('admin.perk.reactivate', $item->id)}}"
    ...
    <div id="popup-modal" tabindex="-1"
        ...
        <form method="post" action="{{route('admin.perk.destroy', $item->id)}}">
            @method('delete')
            @csrf
